...
**Stage 3 — Enclosure, deployed.** 3D-print the PETG enclosure from [`enclosure/`](enclosure/) — a two-part "gourd": cone spire, continuous skirt vents, and a foot sized for the full Grove HM3301 module (the 80×40 carrier board, not just the sensor can), breathing face-down through mesh-screened floor windows. Support-free, screw-locked joint, covers both variants. Design rationale and assembly guide live in that folder; the Bali deployment notes below still apply. Mount it under shade, never in direct sun. Power via a weatherproof USB supply or a 5V solar panel + buck converter (the latter is a separate project; start with wall power).
