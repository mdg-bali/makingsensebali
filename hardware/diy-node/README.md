...
| 1 | Solderless breadboard (prototyping) + perfboard 4×6 cm (deployment) | Tokopedia "PCB matrix board" | 4×6 fits the XIAO + BME680 with room to spare and matches the printed enclosure. A 5×7 build still works — regenerate the enclosure STLs with `pb_w=50, pb_h=70`. Or skip straight to a printed PCB on Fab Lab Bali's mill for batch builds. |
...
**Stage 2 — Perfboard, USB-powered, indoor.** Solder onto a 4×6 cm matrix board (the size the printed enclosure assumes). Use **female headers** for the XIAO and the BME680 — they're the parts most likely to die from a wiring mistake or surge, and you want to swap without desoldering. For the Plus build, solder four wires to the HM3301 carrier's test pads rather than using the Grove cable — the enclosure mounts the module socket-down (see `enclosure/`). Run it for 48 hours indoors next to a known reference (a phone's air quality app pointed at a window will do for a sanity check). Confirm readings are stable, the device doesn't reset, and MQTT reconnects after WiFi drops.

**Stage 3 — Enclosure, deployed.** 3D-print the PETG enclosure from [`enclosure/`](enclosure/) — the "pine cone": a biomimetic scaled skin that is simultaneously the rain shedding and the ventilation, the Grove HM3301 standing vertical behind a barred grille, component locations marked by printed footprint outlines instead of text, and an optional LiPo bay. Support-free, screw-locked joint, covers both variants; the four earlier designs live in `enclosure/archive/` with a disclaimer. Design rationale and assembly guide in that folder; the Bali deployment notes below still apply. Mount it under shade, never in direct sun. Power via a weatherproof USB supply or a 5V solar panel + buck converter (the latter is a separate project; start with wall power).
