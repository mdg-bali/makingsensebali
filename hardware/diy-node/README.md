...
**Stage 3 — Enclosure, deployed.** 3D-print the PETG enclosure from [`enclosure/`](enclosure/) — a two-part "column": cone spire, skirt vents, the full Grove HM3301 module standing vertical behind a louvered grille, printed labels showing where every component goes, and an optional LiPo bay. Support-free, screw-locked joint, covers both variants; superseded designs live in `enclosure/archive/` with a disclaimer. Design rationale and assembly guide in that folder; the Bali deployment notes below still apply. Mount it under shade, never in direct sun. Power via a weatherproof USB supply or a 5V solar panel + buck converter (the latter is a separate project; start with wall power).
