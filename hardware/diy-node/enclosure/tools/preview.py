#!/usr/bin/env python3
"""
Render preview images of a 3D model (STL) for visual inspection.
...
Usage:
    ...
    python3 preview.py model.stl --strict             # exit 2 if not watertight

Dependencies: pip install trimesh pyrender Pillow
Source: adapted from flowful-ai/cad-skill (MIT). Toolchain-agnostic (works
on STLs from build123d, CadQuery, OpenSCAD, anything).
"""
import sys
import os
import argparse
...
def _render_frame(scene, radius, center, elev, azim, renderer):
    ...
    distance = radius / math.sin(yfov / 2) * 1.15
...
def _info_lines(tm):
    ...
    try:
        vol = abs(tm.volume)
        weight_g = vol / 1000 * 1.27  # PETG density ~1.27 g/cm3
        line2 += f"  |  PETG estimate: ~{weight_g:.0f} g"
    except Exception:
        pass
    return line1, line2
